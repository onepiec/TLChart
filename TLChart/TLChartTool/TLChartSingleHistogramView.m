...
@interface TLChartSingleHistogramView()


@property (nonatomic,weak) CALayer * contentLayer;
@property (nonatomic,assign)CGFloat allLineWidth;

@property (nonatomic,weak)CALayer               *animationLayer;              //动画效果mask
@property (nonatomic,strong)CABasicAnimation    *animation;
@end
@implementation TLChartSingleHistogramView

- (CABasicAnimation *)animation{
    if (!_animation) {
        _animation = [CABasicAnimation animationWithKeyPath:@"position"];
        _animation.duration = 0.8;
        _animation.fromValue = [NSValue valueWithCGPoint:_animationLayer.position];
        _animation.toValue = [NSValue valueWithCGPoint:CGPointMake(_animationLayer.position.x +CGRectGetWidth(self.frame), _animationLayer.position.y)];
...
- (CALayer *)animationLayer{
    if (!_animationLayer) {
        ...
        animationLayer.frame = CGRectMake(_verTitleWidth, 0, CGRectGetWidth(self.frame) -_verTitleWidth, CGRectGetHeight(self.frame) -_horTitleHeight- _lineWidth);
...
- (void)choseItemAction:(UITapGestureRecognizer *)tapGesture{
    ...
    for (int i =0; i <self.dataArr.count; i ++) {
        ...
        CGFloat itemtWidth = _allLineWidth *[self.dataArr[i] floatValue];
        
        if ((tapPoint.x >= _verTitleWidth && tapPoint.x <= _verTitleWidth +itemtWidth) &&
            (tapPoint.y >= cellHeight/4 +cellHeight *i && tapPoint.y <= cellHeight *3/4 +cellHeight *i)) {
...
- (void)addAnimation{
    ...
    [self.contentLayer addSublayer:self.animationLayer];
    
    [self.animationLayer removeAnimationForKey:@"position-x"];
    [self.animationLayer addAnimation:self.animation forKey:@"position-x"];
...
- (void)tl_layoutContent{
    ...
    for (int i =0; i <self.dataArr.count; i ++) {
        ...
        if (self.dataColorArr.count >0) {
            contentColor = self.dataColorArr[i];
        }
        [contentColor setFill];
        
        CGFloat itemWidth = _allLineWidth *[self.dataArr[i] floatValue];
        
        CGContextMoveToPoint(context, _verTitleWidth, cellHeight/4 +cellHeight *i);
        
        CGContextAddArcToPoint(context, _verTitleWidth +itemWidth, cellHeight/4 +cellHeight *i, _verTitleWidth +itemWidth, cellHeight/4 +cellHeight *i +_corner *2, _corner);
        CGContextAddArcToPoint(context, _verTitleWidth +itemWidth, cellHeight *3/4 +cellHeight *i, _verTitleWidth +itemWidth -_corner *2, cellHeight *3/4 +cellHeight *i, _corner);
        
        CGContextAddLineToPoint(context,_verTitleWidth, cellHeight *3/4 +cellHeight *i);
        CGContextFillPath(context);
        
        //画柱状图里面的文字
        NSDictionary *attributes = @{ NSFontAttributeName:[UIFont systemFontOfSize:_contentFont],NSForegroundColorAttributeName:_contentColor,NSParagraphStyleAttributeName:paragraphStyle};
        
        NSString *title = self.dataTitleArr[i];
        CGSize strSize = [title sizeWithAttributes:attributes];
        [title drawInRect:CGRectMake(_verTitleWidth, cellHeight/4 +cellHeight *i +(cellHeight/2 -strSize.height)/2, itemWidth, strSize.height) withAttributes:attributes];
    }
